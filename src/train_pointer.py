"""
Training Pipeline for the Pointer MCM Network.
================================================
4-stage curriculum learning: tiny → short → medium → full chains.

Usage:
    python -m src.train_pointer
    python -m src.train_pointer --stage 1      # run only stage 1
"""
...
def evaluate_cost_accuracy(model, loader, device, max_samples=2000):
    ...
    return mape, exact_rate, avg_split_acc
...
def train_stage(model, stage, device, resume_path=None):
    ...
    train_loader, val_loader, test_loader, _ = create_pointer_dataloaders(
        DATA_PATH, batch_size=BATCH_SIZE, max_chain_len=cfg['max_n']
    )
    ...
    no_improve = 0
    save_path = os.path.join(MODEL_DIR, f'pointer_stage{stage}.pth')
    best_path = os.path.join(MODEL_DIR, 'pointer_best.pth')
    ...
    mape, exact_rate, split_acc = evaluate_cost_accuracy(model, test_loader, device)
    print(f"  Split Accuracy: {split_acc:.2f}%")
    print(f"  Cost MAPE: {mape:.2f}%")
    print(f"  Exact Match Rate (<0.1% error): {exact_rate:.1f}%")

    return model


def main():
    parser = argparse.ArgumentParser(description='Train Pointer MCM Network')
    ...
    if args.stage == 0:
        ...
        for stage in range(1, 5):
            ...
            resume_path = os.path.join(MODEL_DIR, f'pointer_stage{stage}.pth')
    else:
        # Run specific stage
        if args.stage > 1 and resume_path is None:
            prev = os.path.join(MODEL_DIR, f'pointer_stage{args.stage - 1}.pth')
    ...
    print(f"\n{'=' * 70}")
    print(f"  FINAL EVALUATION (Full Test Set)")
    print(f"{'=' * 70}\n")
    ...
    if mape < 5.0:
        ...
        print(f"  GOOD: MAPE is {mape:.2f}% -- significant improvement over old models!")
    else:
        print(f"  RESULT: MAPE is {mape:.2f}%. Consider more training or architecture tuning.")
