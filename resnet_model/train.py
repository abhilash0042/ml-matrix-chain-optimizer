"""
RESNET — MCM Cost Predictor (Retrained)
=======================================
Uses 131 features, 8 ResBlocks, MAPE+LogCosh hybrid loss, OneCycleLR.
"""
...
def train_resnet():
    ...
    print("=" * 60)
    print("  RESNET — RETRAINED (131 features, 8 ResBlocks, MAPE)")
    ...
    scaler_path = os.path.join(os.path.dirname(__file__), '..', 'models', 'resnet_scaler.joblib')
    ...
    model = MCMResNet(X_train.shape[1], hidden_size=512, num_blocks=8, dropout=0.1).to(device)
    ...
    # 5. Training Loop
    print(f"\n[4/5] Training for up to {epochs} epochs (patience=80)...")
    best_val = float('inf')
    patience = 0
    max_patience = 80
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 25 == 0:
            ...
            print(f"  Epoch {epoch+1:4d}/{epochs} | Train MAPE: {np.mean(train_mapes):.4f} | Val MAPE: {avg_val:.4f} | LR: {lr_now:.2e}")
    ...
    with torch.no_grad():
        ...
        for bx, by in DataLoader(test_ds, batch_size=512):
            bx = bx.to(device)
    ...
    results_path = os.path.join(os.path.dirname(__file__), '..', 'models', 'resnet_results.json')
