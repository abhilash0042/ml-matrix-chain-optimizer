...
algorithm's intermediate state generalize far better than unaligned models. Our
split-table prediction task is an instance of this paradigm applied to the MCM
dynamic program.
...
reasoning that algorithm-aligned architectures extrapolate in size~\cite{b26}.
...
Hu--Shing        & $O(n\log n)$  & --- & \multicolumn{2}{c}{not benchmarked} \\
...
optimizers~\cite{b17}. The Hu--Shing algorithm~\cite{b2} achieves $O(n\log n)$
and would dominate single-chain latency at large $n$; benchmarking it is planned
...
\noindent\textbf{Synthetic data.} All training and evaluation chains are synthetic
($d\in[10,500]$ and structured variants). Real workloads---tensor contraction
sequences in deep learning compilers, join orders in query optimizers---may have
dimension statistics outside this range. Our accuracy and validity conclusions are
explicitly conditioned on the studied distributions; validating on traces from
production compilers (e.g., TVM tensor programs) is the most important item of
future work.

\noindent\textbf{No Hu--Shing runtime benchmark.} We compare against $O(n^3)$ DP
because it is the de facto standard in the systems that motivate this work, but a
rigorous latency study should include an optimized Hu--Shing implementation and
optimized C/C++ DP. Until then, our speedup figures should be read as
``vs.\ Python DP.''

\noindent\textbf{Speedup granularity.} We report aggregate speedup factors rather
than a per-model millisecond latency table with confidence intervals; producing
such a table is planned.
...
checkpoint per model. Per-seed variance and convergence curves are not reported
and would strengthen the empirical claims.
...
costs directly. Evaluation across 500 diverse synthetic chains reveals a stark
...
framing simultaneously reduces error to 0.029\% MAPE while providing a measured
$16{\times}$ speedup over the Python DP reference.
...
algorithmic reasoning~\cite{b25,b26}: embedding algorithmic structure directly
into neural architectures is a prerequisite for reliable learned combinatorial
optimization. Future work will evaluate on real compiler traces, benchmark against
optimized exact algorithms including Hu--Shing, and scale the structural approach
to larger tensor computation graphs.
